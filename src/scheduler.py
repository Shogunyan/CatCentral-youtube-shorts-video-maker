...
class Pipeline:
    ...
    def run(self) -> bool:
        ...
        # Accept 2+ clips — the source Short determines how many segments exist.
        # A ranking video with 2-4 clips is still valid and looks great.
        min_clips = 2
        if len(downloaded) < min_clips:
            ...
            return False
        if len(downloaded) < n:
            logger.info(f"Proceeding with {len(downloaded)}/{n} clips")
