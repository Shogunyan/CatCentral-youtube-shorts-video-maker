...
def _write_env(
    ...
    # Preserve existing secrets if the caller doesn't supply new ones.
    if ENV_PATH.exists():
        for line in ENV_PATH.read_text().splitlines():
            if not gemini_api_key and line.startswith("GEMINI_API_KEY="):
                gemini_api_key = line.split("=", 1)[1].strip()
            if not instagram_username and line.startswith("INSTAGRAM_USERNAME="):
                instagram_username = line.split("=", 1)[1].strip()
            if not instagram_password and line.startswith("INSTAGRAM_PASSWORD="):
                instagram_password = line.split("=", 1)[1].strip()
...
LOG_LEVEL=INFO
# Optional: Gemini API key enables AI visual clip-source detection.
